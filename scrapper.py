...
Front = {'VueJS', 'JS', 'JavaScript', 'HTML', 'CSS', 'ReactJS', 'Angular', 'ReactNative', 'TypeScript', 'Node.JS'}
...
def scraper():
  for n, i in enumerate(quotes, start=1):
      ...
      itemTagsNoneFormat = i.find('div', class_='tm-article-snippet__hubs')  # tags
      if itemTagsNoneFormat is not None:
        itemTagsProm = itemTagsNoneFormat.replace('#', '')
      else:
        itemTagsProm = 'Отсутсвует'
      itemTags = itemTagsProm.replace('*', '')
